class RC5:
    ...
    def __init__(self, w, R, key, strip_extra_nulls=False):
        ...
        self.encryptedMessage = b''
        # al = self.encryptFile("Зашифрованное сообщение")
        # print(al)
        # print(self.decryptFile(al))
    ...
    def encryptFile(self, text):
        ...
        for i in range(0, len(text_bytes), self.w4):
            ...
            # Шифруем блок и добавляем его к результату
        ...
        self.encryptedMessage = encrypted_data
        return encrypted_data

    def decryptFile(self, dataFromWindow):
        data = self.encryptedMessage
        res, run = b'', True
        while run:
            temp = data[:self.w4]
            if len(temp) != self.w4:
...
if (__name__ == "__main__"):
    RC5(key="КодовоеСлово", R=12, w = 32)